---
title: "Exon-only main-chromosome interaction model"
...
params:
  ...
  alpha: 0.05
  lfc_threshold: 1
  output_tag: "exon-only-interaction-model"
---

## Purpose

This page reruns the complete main-chromosome interaction model from exon-only
featureCounts. All 45 samples, the rRNA exclusion list, placement filter,
expression filter, model formula, and significance thresholds match the primary
transcript-plus-exon page.
...
Test diet, infection, and diet-by-infection effects with all 45 samples.
:::
::: {.walkthrough-step}
**Step 3 - Interpret structure**

Use coefficients and PCA to summarize the conservative host signal.
:::
:::
...
## Omnibus diet-by-infection interaction test

The coefficient tables below test particular diet comparisons. In parallel, a
likelihood-ratio test compares the complete model with a model lacking every
diet-by-infection term. This provides one gene-level test of whether allowing
infection responses to differ among diets improves model fit at all.
...
lrt_results <- as.data.frame(
  DESeq2::results(dds_lrt, alpha = params$alpha, cooksCutoff = FALSE)
)
lrt_results$gene_id <- rownames(lrt_results)
lrt_results$significant <- !is.na(lrt_results$padj) &
  lrt_results$padj < params$alpha
lrt_results <- lrt_results[, c(
  "gene_id", "baseMean", "log2FoldChange", "lfcSE", "stat", "pvalue",
  "padj", "significant"
)]

lrt_summary <- data.frame(
  count_definition = "Exon only",
  samples = ncol(counts),
  genes_tested = nrow(lrt_results),
  omnibus_interaction_FDR_lt_0_05 = sum(lrt_results$significant, na.rm = TRUE)
)

write.csv(
  lrt_results,
  file.path(out_dir, "exon_only_omnibus_interaction_lrt_results.csv"),
  row.names = FALSE
)
write.csv(
  lrt_summary,
  file.path(out_dir, "exon_only_omnibus_interaction_lrt_summary.csv"),
  row.names = FALSE
)
knitr::kable(
  lrt_summary,
  caption = "Exon-only omnibus diet-by-infection likelihood-ratio test"
)
```
