...
table. All 44 fat body samples are retained, annotated rRNA loci are excluded,
...
publication_default_params <- list(
  metadata_file = "data/metadata/mehreen_metadata_fixed.txt",
  counts_file = paste0(
    "output/rmd_runs/transcript-exon-primary-deseq2_20260806-004620/",
...
circled_cluster_dir <- latest_dir("^main-chromosome-cluster-enrichment_")
...
curated_target_dir <- latest_dir("^curated-target-gene-figure_")
...
DEG status from the current competitive-host analysis.
The 99 retained target genes stay in the denominator for every contrast; a gene
is shown as `N.S.` when it does not meet the current adjusted-P-value and fold-
change thresholds. A matched all-scaffold view restores all 163 original
curated targets, including the 64 targets on unplaced scaffolds, for direct
comparison with the earlier figure. Placement details remain searchable on the
