...
params:
  ...
  manual_candidate_file: "data/external/curated_target_genes/2026-08-09_Immune_Protein_Anabolism_DEGs_v24.xlsx"
...
visible. Venn membership ignores direction.

Panel C asks whether temporary semi-isolation, which was required by the
infection protocol, introduced an early density-dependent expression response
that could partly confound the diet-by-infection analysis. This question was
motivated by the strong expression of some DEG groups in only a few individuals.
The union of the infection-within-diet DEGs is compared with independent
solitarious-versus-gregarious DEG sets from head and thorax. The reference study
also used female final-instar *S. gregaria* sampled three days post-molt and the
same total-RNA, rRNA-depletion library-preparation workflow. This matching
reduces differences attributable to sex, developmental stage, and library
preparation, although the tissues and experiments remain distinct.

Genes shared with both head and thorax provide the most conservative evidence
that a broader cross-tissue phase-associated response could contribute to the
apparent infection signal. Genes shared with only one reference tissue provide
weaker, tissue-dependent evidence of potential confounding. These genes may
respond to infection, reduced social density, or both, and the overlap cannot
separate those causes. Genes absent from both sets lack phase-reference support,
but are not automatically infection-specific because the independent phase
study did not include fat body. The comparison is therefore interpretive and
cannot establish that the experimental animals underwent a phase transition.
Every set in these panels is restricted to genes on the official nuclear
chromosomes. The broader four-context Venn is retained as a standalone
alternative export.
...
DEG status from the corrected 44-sample competitive-host transcript+exon
analysis. The supplied v21 workbook contains 111 unique targets (77 immune and
34 protein-anabolism genes), and all 111 map to main nuclear chromosomes. Each
...
[curated target-gene analysis page](30-curated-target-gene-figure.html).
...
## Figure S3: gene-level responses of physiological candidates

Two versions are exported from the same statistical results. The first colors
only the fixed set of 115 **Manual curation candidate** genes. The second colors
all 366 physiological candidates: the 115 manual candidates plus 251
**eggNOG-supported candidate** genes. The eggNOG-supported tier retains genes
only when they are absent from the manual set, significant in at least one
displayed contrast, and supported by an explicit eggNOG description, name, GO
term, KEGG annotation, or PFAM domain. Dark candidate points denote manual
candidates; lighter points denote eggNOG-supported candidates. The source table
records the exact field and keyword supporting every eggNOG-based assignment.

Panel A shows the averaged infection effect followed by infection within each
diet. Panels B and C show the three direct diet comparisons among control and
infected locusts, respectively. Every tested main-chromosome, non-rRNA gene
remains visible: pale gray points are non-significant genes, dark gray points
are other significant DEGs, and larger colored circles are physiological
candidates.
Positive fold changes indicate higher expression in infected locusts or in the
first-listed diet; negative values indicate higher expression in controls or in
the second-listed diet. Vertical dashed lines mark absolute log2 fold change of
1, and the horizontal dashed line marks adjusted P = 0.05. Because candidate
themes were informed by annotation review of this dataset, the visualization is
exploratory rather than an independent enrichment test.

Labels follow an objective rule. Among significant physiological candidates
with a mean normalized count of at least 10 and an informative description, up
to three genes are labelled per contrast, with manual curation candidates ranked
before eggNOG-supported candidates and then by absolute unshrunken log2 fold change.
Adjusted P values determine significance; the label ranking is descriptive and
does not constitute an additional statistical test.

```{r figure-s3-volcano-data}
...
```{r figure-s3-volcano-plots, include=FALSE}
make_s3_volcano <- function(plot_data, label_data, panel_title, ncol,
                            theme_breaks, evidence_breaks) {
  label_data <- label_data |>
    dplyr::filter(contrast_id %in% unique(plot_data$contrast_id))
  x_limit <- max(1, ceiling(max(abs(plot_data$log2FoldChange), na.rm = TRUE)))
  ...
  ggplot2::ggplot(
    ...
    ggplot2::geom_hline(
      yintercept = -log10(publication_params$alpha),
      linetype = "dashed",
      linewidth = 0.42,
      color = "#6B7280"
    ) +
    ggplot2::geom_vline(
      xintercept = c(-publication_params$lfc_threshold,
                     publication_params$lfc_threshold),
      linetype = "dashed",
      linewidth = 0.42,
      color = "#6B7280"
    ) +
    ...
    ggplot2::geom_point(
      data = plot_data |>
        dplyr::filter(significant, !functional_highlight),
      color = "#4B5563",
      size = 0.82,
      alpha = 0.58
    ) +
    ggplot2::geom_point(
      data = plot_data |>
        dplyr::filter(functional_highlight),
      ggplot2::aes(fill = candidate_theme, alpha = evidence_tier),
      shape = 21,
      color = "#111827",
      stroke = 0.38,
      size = 2.45
    ...
    ggrepel::geom_label_repel(
      data = label_data,
      ggplot2::aes(label = candidate_label),
      size = 3.65,
      label.size = 0.16,
    ...
    ggplot2::scale_fill_manual(
      values = s3_candidate_theme_colors,
      breaks = theme_breaks,
      limits = theme_breaks,
      drop = FALSE
    ) +
    ggplot2::scale_alpha_manual(
      values = s3_evidence_alpha,
      breaks = evidence_breaks,
      limits = evidence_breaks,
      drop = FALSE
    ) +
    theme_pub(base_size = 15) +
    ggplot2::theme(
      strip.text = ggplot2::element_text(size = 14.5, face = "bold"),
      axis.text = ggplot2::element_text(size = 12.5, color = "#111827"),
    ...
    ggplot2::guides(
      fill = ggplot2::guide_legend(
        nrow = 2, order = 1,
        byrow = TRUE,
        override.aes = list(shape = 21, size = 5, color = "#111827")
      ),
      alpha = ggplot2::guide_legend(
        nrow = 1, order = 2,
        override.aes = list(shape = 21, size = 5, fill = "#4B5563")
      )
    ) +
    ggplot2::labs(
      title = panel_title,
      x = "Unshrunken log2 fold change: numerator relative to denominator",
      y = expression(-log[10](adjusted~italic(P))),
      fill = "Physiological category",
      alpha = "Candidate evidence"
    )
}

s3_build_version <- function(plot_data, label_data, export_prefix,
                             figure_title,
                             theme_breaks = s3_candidate_theme_levels,
                             evidence_breaks = s3_evidence_levels) {
  ...
  panel_b_data <- plot_data |>
    dplyr::filter(stringr::str_starts(as.character(contrast_id), "control_diet"))
  panel_c_data <- plot_data |>
    dplyr::filter(stringr::str_starts(as.character(contrast_id), "infected_diet"))

  panel_a <- make_s3_volcano(
    panel_a_data, label_data, "A. Infection-responsive genes", 2,
    theme_breaks, evidence_breaks
  )
  panel_b <- make_s3_volcano(
    panel_b_data, label_data, "B. Diet-associated genes among controls", 3,
    theme_breaks, evidence_breaks
  )
  panel_c <- make_s3_volcano(
    panel_c_data, label_data,
    "C. Diet-associated genes among infected locusts", 3,
    theme_breaks, evidence_breaks
  )

  save_png_svg(
    panel_a,
    paste0(export_prefix, "_panelA_infection_volcanoes"),
    width = 14.5,
    height = 13.5
  )
  save_png_svg(
    panel_b,
    paste0(export_prefix, "_panelB_control_diet_volcanoes"),
    width = 18,
    height = 9.2
  )
  save_png_svg(
    panel_c,
    paste0(export_prefix, "_panelC_infected_diet_volcanoes"),
    width = 18,
    height = 9.2
  )
  ...
  plot_body <- patchwork::wrap_plots(
    ...
    ncol = 1,
    heights = c(0.44, 0.28, 0.28)
  )

  composite <- patchwork::wrap_plots(
    plot_body,
    patchwork::wrap_elements(full = shared_legend),
    ncol = 1,
    heights = c(0.94, 0.06)
  )
  ...
  composite <- composite +
    patchwork::plot_annotation(
      title = figure_title,
      subtitle = paste0(
        "Primary transcript-plus-exon analysis; 44 QC-passed libraries; ",
        "rRNA excluded; nuclear chromosomes only"
      ),
      theme = ggplot2::theme(
  ...
  save_png_svg(
    composite,
    paste0(export_prefix, "_candidate_volcanoes"),
    width = 20,
    height = 26
  )

  list(panel_a = panel_a, panel_b = panel_b, panel_c = panel_c,
       composite = composite)
}

s3_manual_115_version <- s3_build_version(
  s3_manual_plot_data,
  s3_manual_label_data,
  "figureS3_manual_115",
  "Figure S3. Gene-level responses of 115 manual curation candidates",
  theme_breaks = c(
    "Immune pathways",
    "Protein storage and synthesis"
  ),
  evidence_breaks = "Manual curation candidate"
)

s3_manual_plus_eggnog_366_version <- s3_build_version(
  s3_candidate_volcano_data,
  s3_candidate_label_data,
  "figureS3_manual_plus_eggnog_366",
  paste(
    "Figure S3. Gene-level responses of manual and",
    "eggNOG-supported physiological candidates"
  )
)
```

### Manual curation candidates (115 genes)

```{r figure-s3-manual-display, echo=FALSE, fig.width=18, fig.height=26}
s3_manual_115_version$composite
```

### Manual and eggNOG-supported candidates (366 genes)

```{r figure-s3-evidence-tiered-display, echo=FALSE, fig.width=18, fig.height=26}
s3_manual_plus_eggnog_366_version$composite
```

```{r figure-s3-export-manifest, echo=FALSE}
s3_export_files <- c(
...
knitr::kable(
  tibble::tibble(
    file = s3_export_files,
    present = file.exists(file.path(out_dir, s3_export_files))
  ),
  caption = "Figure S3 plot and source-data exports"
)
```

## Figure S4: GO enrichment of diet-resolved responses

Figure S4 summarizes GO enrichment among genes induced by infection within
each diet, the subset of induced genes detected as significant in only one
diet, and direct diet contrasts among control locusts. “Diet-exclusive” refers
to the pattern of DEG detection and is interpreted as exploratory: failure to
reach the DEG threshold in another diet is not itself evidence for a
significant diet-by-infection interaction. The formal interaction test remains
the appropriate test of whether infection effects differ among diets.

```{r figure-s4-diet-resolved-go, echo=FALSE, out.width="92%"}
go_enrichment_dir <- latest_dir("^go-term-enrichment_")
s4_go_files <- c(
...
s4_go_sources <- file.path(go_enrichment_dir, s4_go_files)
if (!all(file.exists(s4_go_sources))) {
  stop("Missing Figure S4 GO-enrichment source exports in: ",
       go_enrichment_dir)
}
s4_go_targets <- file.path(out_dir, basename(s4_go_sources))
file.copy(s4_go_sources, s4_go_targets, overwrite = TRUE)

grid::grid.newpage()
grid::grid.raster(png::readPNG(s4_go_sources[[1]]))
```

```{r figure-s4-go-export-manifest, echo=FALSE}
knitr::kable(
  tibble::tibble(
    file = basename(s4_go_targets),
    present = file.exists(s4_go_targets)
  ),
  caption = "Figure S4 GO-enrichment plot and source-data exports"
)
```

<!-- Chromosomal DEG localization remains an unnumbered candidate supplementary
figure for possible inclusion as Figure S5 or a later supplement. -->
...
## Paper supplementary data workbook

The paper-ready source tables are consolidated in one searchable workbook:
[download the transcript-plus-exon supplementary tables](assets/downloads/paper_supplementary_tables_transcript_exon.xlsx).
It contains the 45-library metadata and mapping-quality record (including the
documented exclusion of library 1044), the complete main-chromosome DEG
catalogue, GO and KEGG enrichment results with their contributing gene IDs,
the physiological-candidate evidence table, and every displayed contrast for
those candidates. The primary tier contains 115 manual curation candidates; the
lighter tier contains 251 eggNOG-supported candidates that were significant in
at least one displayed contrast. Non-significant candidate results are retained in the contrast-level
sheet so that absence of a response in a particular comparison remains visible.
