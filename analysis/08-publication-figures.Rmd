...
publication_default_params <- list(
  metadata_file = "data/metadata/mehreen_metadata_fixed.txt",
  counts_file = paste0(
    "output/runs/host_pathogen_dual_20260727_000952/04-count-matrices/",
...
table is included so a collaborator can verify the exact count matrix, official
placement source, and current DESeq2/cluster outputs used for the export.
...
base_inputs <- c(
  ...
  figure3_reference_source, figure4_curated_source, figure4_placement_source
)
...
Figure 4 preserves the coauthor-curated biological categories but recalculates
every DEG status from the current competitive-host, main-chromosome analysis.
...
figure4_exports <- data.frame(
  ...
  source = basename(c(figure4_curated_source, figure4_placement_source)),
  copied_to = basename(c(figure4_png, figure4_placement_png)),
  present = file.exists(c(figure4_png, figure4_placement_png))
)
