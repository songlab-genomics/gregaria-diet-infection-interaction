...
figure4_unplaced_source <- file.path(
  curated_target_dir,
  "figure4_curated_targets_unplaced_scaffolds.png"
)
...
base_inputs <- c(
  ...
  figure3_reference_source, figure4_curated_source, figure4_unplaced_source,
...
solitarious and gregarious locusts in head and thorax. The broader four-context
Venn is retained as a standalone alternative export.
...
change thresholds. The 64 curated targets on unplaced scaffolds are excluded
from the primary percentages and remain searchable on the
...
figure4_unplaced_png <- copy_publication_asset(
  figure4_unplaced_source,
...
figure4_exports <- data.frame(
  figure = c(
    ...
    "Figure 4 unplaced-scaffold curated targets",
    "Figure 4 placement-impact audit"
  ),
  source = basename(c(
    figure4_curated_source, figure4_unplaced_source,
    figure4_placement_source
  )),
  copied_to = basename(c(
    figure4_png, figure4_unplaced_png, figure4_placement_png
  )),
  present = file.exists(c(
    figure4_png, figure4_unplaced_png, figure4_placement_png
  ))
...
#### Unplaced-scaffold comparison

```{r figure-4-unplaced-display, echo=FALSE, fig.width=15, fig.height=11.5}
if (file.exists(figure4_unplaced_png)) {
  grid::grid.newpage()
  grid::grid.raster(png::readPNG(figure4_unplaced_png))
}
```
