---
title: "Primary DESeq2 analysis and placement sensitivity"
...
params:
  ...
  host_only_counts: "output/rmd_runs/transcript-exon-host-only-excluding1044_20260809-233420/host_counts_no_rrna.tsv"
  ...
  cluster_assignments: "output/rmd_runs/main-chromosome-cluster-enrichment_20260811-121100/full_heatmap_expression_cluster_assignments.csv"
  homology_evidence: "output/rmd_runs/scaffold-origin-audit_20260811-195052/corrected_gene_homology_evidence.tsv"
  alpha: 0.05
  lfc_threshold: 1
  min_count: 10
  min_samples: 3
  output_tag: "placed-unplaced-scaffold-analysis"
---
...
1. **Primary paper analysis:** genes assigned by the official assembly metadata
   to nuclear chromosomes, plus the valid mitochondrial genes, are tested as
   the primary host-gene hypothesis family.
2. **Exploratory sensitivity analysis:** genes on officially unplaced scaffolds
...
### Continue through the primary results

This page is the main entry point for the transcript-plus-exon analysis. The
reader-facing results continue with [infection responses within each
diet](16-all-samples-infection-within-diet-deg.html), [DEG
overlap](07-deg-overlap-prioritization.html), the [searchable DEG
catalogue](24-all-deg-catalogue.html), [GO and KEGG
enrichment](10-go-term-enrichment.html), and [expression
clusters](26-circled-cluster-enrichment.html).

The matched all-scaffold calculations remain available as detailed notebooks
for the [interaction model](04-deseq2-interaction-model.html), [diet
contrasts](05-diet-pairwise-deg.html), [infection
contrasts](06-infection-within-diet-deg.html), [overlap](31-all-scaffolds-deg-overlap-prioritization.html),
[catalogue](32-all-scaffolds-deg-catalogue.html), and [functional
enrichment](33-all-scaffolds-go-term-enrichment.html). They are validation
views rather than a second competing narrative.
...
placement <- readr::read_tsv(input_files[["placement"]], show_col_types = FALSE) |>
  ...
  dplyr::mutate(
    analysis_scope = dplyr::case_when(
      ...
      .data$sequence_class %in% c("Nuclear chromosome", "Mitochondrial") ~ "Placed/host-assigned",
      TRUE ~ NA_character_
    ),
...
prepare_branch_counts <- function(counts, branch) {
  ...
  retained_ids <- setdiff(placement_ids, rrna_ids)
