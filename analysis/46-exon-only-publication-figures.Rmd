...
table. All 45 fat body samples are retained, annotated rRNA loci are excluded,
...
publication_default_params <- list(
  metadata_file = "data/metadata/mehreen_metadata_corrected_45.txt",
  counts_file = paste0(
    "output/runs/host_pathogen_dual_corrected45_20260809-153146/04-count-matrices/",
...
not repeated here.
The primary KEGG panel presents the eukaryote-inclusive pathways around a
locust illustration. Each pathway is connected to a conceptual physiological
region and reports its `-log10` adjusted P-value and number of cluster genes
below the pathway name. Connector positions aid interpretation; they do not
represent measured sub-tissue localization. The pathway set retains
eukaryote/metazoan-compatible, conserved cross-domain, and broad reference maps
while excluding explicitly microbial/prokaryotic and disease-reference labels.
The complete figure is provided in two matched versions: one with KEGG
lollipops and one with the locust pathway map. The strict insect-focused version
remains as a secondary export. The complete pathway-scope and pathway-gene
...
DEG status from the corrected 45-sample competitive-host exon-only analysis.
