...
no_outlier_results <- fit_direct_scenario(
  current_outliers,
  "Current outliers removed"
)
scenario_results <- dplyr::bind_rows(all_sample_results, no_outlier_results)
...
scenario_sample_counts <- dplyr::bind_rows(
  metadata_for_models |>
    dplyr::count(Diet, Treatment, name = "n_samples") |>
    dplyr::mutate(scenario = "All samples"),
  metadata_for_models |>
    dplyr::filter(!label %in% current_outliers) |>
    dplyr::count(Diet, Treatment, name = "n_samples") |>
    dplyr::mutate(scenario = "Current outliers removed")
)
...
Figure S2 maps the primary main-chromosome DEGs from each broad context onto the
chromosome coordinates in the RefSeq GFF. Unplaced scaffolds are outside this
publication figure by design and remain documented on the scaffold-origin and
placement-sensitivity pages.
...
## Figure S3: mapping-quality basis for excluding 1044

Figure S3 preserves all 45 sequenced libraries for the QC comparison, while all
biological models elsewhere on this page use the 44-library cohort. Panel A
shows unique mapping and Panel B shows unique plus accepted multimapping; the
factorial diet, infection, and interaction tests are printed on the panels.

```{r figure-s3-mapping-quality, echo=FALSE, out.width="82%"}
...
## Figure S4: individual mass and mapping quality

Figure S4 relates the mass recorded for each individual before extraction to
the competitive STAR mapping percentages. Panel A uses unique mapping and Panel
B adds reads accepted at 2--20 loci. All 45 libraries remain visible for this
QC comparison, with 1044 highlighted. Solid fits use all libraries and dashed
fits omit 1044. Each panel reports Spearman tests and mass effects adjusted for
diet and infection; neither mapping definition shows a supported mass effect.

```{r figure-s4-mapping-mass, echo=FALSE, out.width="82%"}
qc_dir <- latest_dir("^count-qc-exploration_")
s4_files <- c(
  "figureS4_mapping_quality_by_individual_mass.png",
  "figureS4_mapping_quality_by_individual_mass.svg",
  "figureS4_mapping_quality_by_individual_mass.pdf",
  "mapping_quality_mass_association_tests.csv",
  "mapping_quality_and_individual_mass_45_samples.csv"
)
s4_sources <- file.path(qc_dir, s4_files)
if (!all(file.exists(s4_sources))) {
  stop("Missing Figure S4 mapping-mass exports in: ", qc_dir)
}
s4_targets <- file.path(out_dir, basename(s4_sources))
file.copy(s4_sources, s4_targets, overwrite = TRUE)
s4_png_source <- s4_sources[grepl("[.]png$", s4_sources)]
grid::grid.newpage()
grid::grid.raster(png::readPNG(s4_png_source))
```

```{r figure-s4-export-manifest, echo=FALSE}
knitr::kable(
  tibble::tibble(file = basename(s4_targets), present = file.exists(s4_targets)),
  caption = "Figure S4 individual-mass and mapping-quality exports"
)
```
