...
outDir <- file.path(workDir, "rRNA_removed/DEG_ControlOnly_DietEffects_SVA_NO_OUTLIERS")
...
target <- target %>% filter(Treatment == "Control")
...
make_full_heatmap <- function(dds, vsd, outDir) {
  ...
  # 🔹 Use top variable genes across ALL Control samples
  ...
  pheatmap::pheatmap(
    ...
    filename = file.path(outDir, "Heatmap_ALL_Control_no_rRNA.pdf")
  )
}
...
p_pca <- ggplot(pca_data, aes(PC1, PC2, color = Diet)) +
  ...
  theme_minimal() +
  labs(
    title = "PCA - Control Only",
    x = paste0("PC1: ", percentVar[1], "%"),
    y = paste0("PC2: ", percentVar[2], "%")
  )

ggsave(file.path(outDir, "PCA_Control_no_rRNA.png"), p_pca, width = 6, height = 5)
...
p_umap <- ggplot(umap_df, aes(UMAP1, UMAP2, color = Diet)) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "UMAP - Control Only")

ggsave(file.path(outDir, "UMAP_Control_no_rRNA.png"), p_umap, width = 6, height = 5)
